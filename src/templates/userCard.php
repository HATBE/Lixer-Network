<div class="card cursor-default">
    ...
    <div class="card-body rounded-circle d-flex flex-column justify-content-center align-items-center">
        <div class="w-50 h-50 border-4 rounded-circle position-relative text-center" >
            ...
            <?php if($user->getOnlineState()['name'] != 'Offline'):?><span title="<?= $user->getOnlineState()['name'];?>" style="background-color: <?= $user->getOnlineState()['color'];?>; width: 20%; height: 20%; bottom: 5%; right: 5%;" class="shadow rounded-circle position-absolute"></span><?php endif;?>
        </div>
        ...
        <div class="user-select-none my-2 d-flex justify-content-between w-100">
            <div>
                <button id="followbtn" data-user="<?= $user->getId();?>" data-following="<?= $loggedInUser->isFollowing($user->getId()) ? 'true' : 'false';?>" class="mx-2-2 btn btn-<?= $loggedInUser->isFollowing($user->getId()) ? 'danger' : 'primary';?>"><?= $loggedInUser->isFollowing($user->getId()) ? 'Unfollow' : 'Follow';?></button>
